...
CAREER_PIVOT_ANALYSIS_PROMPT = """You are analyzing a career pivot opportunity for someone stuck in misalignment.
...
SKILL_EXTRACTION_PROMPT = """Given this user description of their current role and experience, extract and map their transferable skills.
...
THREE_STEP_PLAN_PROMPT = """Create a realistic, low-barrier 3-step plan for {person_name} to explore this career pivot: {target_career}
...
5. Success metric (how they will know it worked)

Format as a numbered list. Be tactical and encouraging.
"""

RESUME_REFRAME_PROMPT = """Help {person_name} reframe their resume for this target role: {target_career}
...
2. Translate pain-points into demonstrated values
...
OUTREACH_MESSAGE_PROMPT = """Draft a warm, authentic outreach message for {person_name} to send to someone in their target role.
...
3. Show you have researched the person/company
...
FREELANCE_LAUNCH_PROMPT = """Help {person_name} create a freelance positioning statement for: {niche}
...
MINDSET_CHECK_PROMPT = """Reflect this back to {person_name} as honest but loving feedback:
...
2. Reality check: What is actually true vs. what is their brain lying about
3. Reframe: How to think about this pivot that is empowering but realistic
